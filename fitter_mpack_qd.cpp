...
// calculate data correlation matrix (store in inv_corr)
  ...
  qd_real normalization;
  if(bootstrap_normalization)
  {
    normalization=qd_real(n_data_sets-1);
  }
  else
  {
    normalization=qd_real(n_data_sets)*qd_real(n_data_sets-1);
  }
...
// calculate inverse correlation matrix inv_corr
  ...
  if( (inv_method==LU_inversion) || (inv_method==off_diagonal_rescale) )
  {
    ...
    {
      mpackint lwork, info;
      mpackint* ipiv=new mpackint[n_fit_points];
...
double fitter::chi_sqr(const vector< double >& params)
{
  ...
  vector< qd_real > temp_vec(n_fit_points, 0.0);
  for(int m=0; m<n_fit_points; ++m)
  {
    temp_vec[m]=average[m]-model->eval_function(m);
  }

  qd_real temp_chi_sqr=0.0;
  qd_real temp_chi_sqr2;
...
void fitter::beta(const vector< double >& params, gsl_vector* result)
{
  ...
  for(int m=0; m<n_fit_points; ++m)
  {
    temp_vec_1[m]=to_double(average[m])-model->eval_function(m);
  }
